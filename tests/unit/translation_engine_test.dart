...
void main() {
  group('OfflineTranslationEngine Tests', () {
    ...
    test(
        'returns original string when source and target language are identical',
        () async {
      final res = await engine.translate(
        'Architecture',
        options: const TranslationOptions(
    ...
    test('applies formality rules when specified in Spanish and German',
        () async {
    ...
    test('handles auto source language detection when source is not specified',
        () async {
      final res = await engine.translate(
        'こんにちは',
    ...
    test(
        'translates tokenized words across non-English pairs and preserves unknown tokens',
        () async {
